...
- [ ] Alineada con arquitectura y stack (Spring Boot 4 + PostgreSQL + RabbitMQ)
- [ ] Dependencias y riesgos identificados
- [ ] Microservicio destino identificado (`employee-service` o `payroll-service`)
...
- `## 2. DISEÑO` — modelos de datos (JPA entities), endpoints API (request/response/HTTP codes), comunicación RabbitMQ
- `## 3. LISTA DE TAREAS` — checklists backend `[ ]`, unit tests `[ ]`, QA `[ ]`
